...
public class BarChartData {
  ...
  public BarChartData(BarChart barChart, Context context){
    this.context = context;
    this.barChart = barChart;
  }

  public void initDataSet(ArrayList<BarEntry> valuesY) {
    if(valuesY.size()>0){
      for(int i = 0 ;i<valuesY.size();i++){
        final BarDataSet set;
        set = new BarDataSet(valuesY, "");
        //set.setMode(BarDataSet.Mode.LINEAR);//類型為折線
        set.setColor(ContextCompat.getColor(context, R.color.main_green));//線的顏色
      ...
      barChart.setNoDataText("暫時沒有數據");
      barChart.setNoDataTextColor(Color.BLUE);//文字顏色
    }
    barChart.invalidate();//繪製圖表
  }

  public void initX(ArrayList dateList) {
    Log.d("LineChart", String.valueOf(barChart));
